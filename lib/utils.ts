...
export async function retry<T>(
  ...
  try {
    ...
    console.error(
      `Error in function call (retry ${attemptCount + 1} of ${maxRetries}):`,
      error,
    );
    ...
    if (attemptCount < maxRetries - 1) {
      ...
      throw new Error(
        `Max retries (${maxRetries}) reached. Unable to complete the function call.`,
      );
    }
  }
}
...
export function parsePlaylistIndex(
  value: string,
...
export function getNextTrackRoute(
  ...
  const nextTrackPlaylistIndex =
    nextTrackIndex === 0
      ? getNextIndex(playlists, playlistIndex)
      : playlistIndex;
...
export function getPreviousTrackRoute(
  ...
  const previousTrackPlaylistIndex =
    trackIndex === 0
      ? getPreviousIndex(playlists, playlistIndex)
      : playlistIndex;
